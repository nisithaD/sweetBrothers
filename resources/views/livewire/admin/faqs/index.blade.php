<div>
    ...
    <a class="btn btn-link btn-color-info btn-active-color-primary me-5 mb-2"
       href="{{asset('/wbpa/faqs/create')}}">Add new</a>
    <table id="kt_datatable_example_5" class="table table-striped table-row-bordered gy-5 gs-7 border rounded">
        ...
        @foreach ($faqs as $faq)
            <tr>
                <td>{{ $faq->title }}</td>
                <td>
                {!! str_limit(strip_tags($faq->content,'100')) !!}
                </td>
                <td>
                    <a href="{{ url('') }}/wbpa/faqs/{{$faq->id}}/edit" class="btn btn-primary btn-sm">
                        <span class="fa fa-edit"></span>
                    </a>
                    <button wire:click="deleteFaq({{$faq->id}})" class="btn btn-danger btn-sm">
